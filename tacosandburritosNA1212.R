...
randomsample <- tacos.trimmed.merge[(sample(1:nrow(tacos.trimmed.merge), 3000)),]
...
X.quanti <- PCAmixdata::splitmix(trainData)$X.quanti
X.quali <- PCAmixdata::splitmix(trainData)$X.quali
tree <- hclustvar(t(X.quanti[,4:133]))
plot(tree)
tree <- as.phylo(tree)
